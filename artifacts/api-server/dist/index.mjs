...
// src/index.ts
var WORKSPACE = path.resolve(import.meta.dirname, "..", "..", "..");
var FRONTEND_DIST = path.join(WORKSPACE, "artifacts", "tma-frontend", "dist");
var FASTAPI_PORT = 8e3;
...
function handleUpgrade(req, socket, head) {
  ...
  const upstream = net.createConnection(FASTAPI_PORT, "127.0.0.1", () => {
    const reqLine = `${req.method ?? "GET"} ${req.url ?? "/"} HTTP/1.1\r
`;
...
function spawnProc(cmd, args, env, label) {
  ...
  child.on("exit", (code, signal) => {
    ...
    setTimeout(() => spawnProc(cmd, args, env, label), 5e3);
  });
...
var hasDist = fs.existsSync(path.join(FRONTEND_DIST, "index.html"));
