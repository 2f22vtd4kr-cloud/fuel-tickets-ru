...
def _ai_rule_based_reply(
    ...
    city_keywords = {
        ...
        "нижн": "Нижний Новгород", "самар": "Самара", "пермь": "Пермь",
        "омск": "Омск", "челябинск": "Челябинск", "красноярск": "Красноярск",
    }

    # ─── Pure geographic query about non-Crimea city ──────────────────────────
    NON_CRIMEA_KEYS = {
        "москв", "мск", "питер", "спб", "ленинград", "казан", "татар",
        "уфа", "екатеринбург", "новосибирск", "краснодар", "ростов", "воронеж",
        "нижн", "самар", "пермь", "омск", "челябинск", "красноярск",
    }
    matched_city_key = next((k for k in city_keywords if k in msg), None)
    if matched_city_key and matched_city_key in NON_CRIMEA_KEYS:
        # Only redirect if no station-search intent is present
        if not any(w in msg for w in ["ближайш", "рядом", "найди", "найти", "где заправ", "азс", "станц", "цена", "стоит", "лимит"]):
            ...
            opener = _rand.choice([
                f"По **{city_name}** — честно, нет данных у меня.",
                f"**{city_name}** вне моей зоны покрытия.",
                f"Хм, по **{city_name}** не могу помочь — я крымский.",
            ])
            reply = (
                f"{opener}\n\n"
                f"«Топливный Узел» работает только по Крыму и Севастополю. "
                f"Сейчас у нас **{green_count}** живых АЗС из {total_stations}.\n\n"
                f"В {city_name} своя сеть АЗС без наших ограничений — там ситуация другая.\n\n"
                f"Расскажу про Крым?"
            )
            return (reply, ["Ситуация в Крыму", "Найти АЗС рядом", "Прогноз кризиса"], None)
